...
public class SwiftTwilioProgrammableVoicePlugin: NSObject, FlutterPlugin,  FlutterStreamHandler, PKPushRegistryDelegate, NotificationDelegate, CallDelegate, AVAudioPlayerDelegate, CXProviderDelegate {
	private var callStatusEventChannelWrapper: CallStatusEventChannelWrapper;
	var _result: FlutterResult?
		private var eventSink: FlutterEventSink?
		// TODO We shouldn't saved our deviceToken, Dart side
		let kCachedDeviceToken = "CachedDeviceToken"
	
		let kClientList = "TwilioContactList"
		private var clients: [String:String]!
		// we can't get rid of this because we makeCall in event (CXStartCallAction)
	  // so we can't just pas the accessToken from Dart to Swift, we need to stock it in the object to use it in event
...
// this is ugly, but I can't delegate this in an external function like
// self.registerVoice(args: args, result: result);
// It's really painfull
					
// I try this but it doesn't work :(
//					guard let token = args["accessToken"] as? String, let deviceToken = deviceToken else {return}
//
//					self.accessToken = token
//
//					TwilioVoice.register(accessToken: token, deviceToken: deviceToken) { (error) in
...
//					}
	...
	func registerVoice(args: Dictionary<String, AnyObject>, result: FlutterResult) {
		// This way of doing things is way way cleaner, but I just can't make it work
	}
				...
				if (self.call != nil && self.call?.state == .connected) {
					...
					print("in else");
						...
						self.checkRecordPermission { (permissionGranted) in
							print("checkRecordPermission cb");
								if (!permissionGranted) {
									print("permission not granted")
										let alertController: UIAlertController = UIAlertController(title: String(format:  NSLocalizedString("mic_permission_title", comment: "") , SwiftTwilioProgrammableVoicePlugin.appName),
...
//				self.sendPhoneCallEvents(description: "Ringing|\(from)|\(callInvite.to)|Incoming", isError: false)
			...
			callStatusEventChannelWrapper.sendCallInvite(callInvite: self.getCallInvitePayload(callInvite: callInvite));
		}
...
//						return
//				}
//		// I have commented the eventSink action because we use the callStatusStream.. to send data to Dart and it make the crash
//				if isError
//				{
//					print("sending error");
